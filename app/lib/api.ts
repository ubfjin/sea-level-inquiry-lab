...
export function flattenGrid(latitudes: number[], longitudes: number[], values: (number | null)[][]): GridPoint[] {
  ...
  latitudes.forEach((lat, i) => longitudes.forEach((lon, j) => {
    ...
    if (value !== null && Number.isFinite(value)) points.push({ lat, lon, value });
  }));
  return points;
}

export type MapResponse = { requested_date: string; data_date: string; latitudes: number[]; longitudes: number[]; values: (number | null)[][]; unit: string };
...
export type TrendResponse = { latitudes: number[]; longitudes: number[]; values: (number | null)[][]; unit: string; area_mean: number };
